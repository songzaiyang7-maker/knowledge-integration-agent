...
输入教材片段：
> 静息电位是指细胞在未受刺激时，细胞膜内外两侧存在的电位差，表现为膜内为负、膜外为正的状态。其产生机制主要与细胞膜上Na⁺-K⁺泵的活动以及膜对K⁺的通透性有关。动作电位是细胞受到刺激后，膜电位发生的一次快速而可逆的倒转，以及随后恢复到原来静息电位水平的过程。

期望输出：
```json
{
  "knowledge_points": [
    {"name": "静息电位", "definition": "细胞在未受刺激时，细胞膜内外两侧存在的电位差，表现为膜内为负、膜外为正的状态。", "category": "核心概念"},
    {"name": "动作电位", "definition": "细胞受到刺激后，膜电位发生的一次快速而可逆的倒转，以及随后恢复到原来静息电位水平的过程。", "category": "核心概念"}
  ],
  "relations": [
    {"source": "静息电位", "target": "动作电位", "relation_type": "prerequisite", "description": "理解动作电位需要先掌握静息电位的概念"}
  ]
}
```

✅ "静息电位"和"动作电位"分别作为独立知识点（可独立考查）
❌ 不要合并为"细胞电位"（太笼统）
❌ 不要拆分为"Na⁺通道开放"（太细，属于动作电位子步骤）"""
...
def process_textbook(parsed_data: dict) -> dict:
    ...
    book_prefix = title[:3]
    llm_calls = 0
    MAX_LLM_CALLS_PER_BOOK = 2  # Limit LLM calls per textbook (reduced for faster startup)
